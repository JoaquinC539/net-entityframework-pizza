...
public class PizzaController : ControllerBase
{
    ...
    public IActionResult Create([FromBody]  PizzaDto pizzaDto)    
    {
        ...
        if(pizzaDto == null)
        {
            return BadRequest("Invalid Pizza data provided");
        }

        var pizza = _service.Create(pizzaDto);
        return CreatedAtAction(nameof(GetById),new {id=pizza!.Id},pizza);
    }
    
    [HttpPut("{id}/addtopping")]
    public IActionResult AddTopping(int id, [FromBody] ToppingIdModel toppingId)
    {
        ...
        {
            _service.AddTopping(id, toppingId.ToppingId);
            return NoContent();    
        }
        else
        {
